...
public class CancelDAO {
	...
	public void cancelReservation() throws NumberFormatException, SQLException, InterruptedException {
		...
		while(true) {
			try {
				reservationID = UserInput.receiveNaturalNumber();
				...
				while(true) {
					if(UserInput.receiveYesOrNo()) {
						...
						Coloring.redOut("약관의 동의를 얻지못해 예매 취소에 실패했습니다. 메인으로 돌아갑니다.");
						Thread.sleep(1500);
		...
		String changeSeatStatus_sql = "UPDATE seats SET `status` = '0' WHERE seat_id = " + seat_id;
		...
		if(UserInput.receiveYesOrNo()) {
			...
			Coloring.redOut("예매를 취소하지 못했습니다. 메인으로 이동합니다.");
		}
		Thread.sleep(1500);
		MainService.mainMenu();
	}
